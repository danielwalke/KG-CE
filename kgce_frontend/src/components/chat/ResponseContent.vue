<template>
    <div class="flex flex-col gap-2 items-start w-full my-2">
        ...
        <button 
            v-if="hasThoughtProcess(msg.content)"
            @click="isShowThinking = !isShowThinking" 
            class="group flex items-center gap-2 px-3 py-2 rounded-md transition-all duration-200 select-none text-sm w-full text-left
            ...
            <span v-if="isGeneratingThought(msg)" class="flex items-center gap-2 ml-auto">
        ...
        <div v-if="isShowThinking && hasThoughtProcess(msg.content)" class="w-full overflow-hidden">
            <div class="text-sm p-4 rounded-md 
                bg-gray-50 text-gray-600 border border-gray-200
                dark:bg-black/20 dark:text-gray-400 dark:border-white/10">
                
                <div v-if="getThoughtContent(msg.content)" v-html="parseMarkdown(getThoughtContent(msg.content))"
        ...
        <div v-html="parseMarkdown(getAnswerContent(msg.content))"
...
const getAnswerContent = (content) => {
    ...
    return content.split('</think>').pop() || '';
};
</script>